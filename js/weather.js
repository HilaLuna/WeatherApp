...
//object'lerde eşittir değil iki nokta koyuyoruz.

//varsayılan şehir ve birim değerlerini atadık.
let currCity = "İstanbul"
let units = "metric"

// hava durumu verilerini API'den alma ve UI'yi güncellemek için asenkron fonksiyon
...
selectors.unitCelsius.addEventListener("click",()=>{
    if(units !== "metric"){ //eğer başlangıç itibariyle metric değilse
        units = "metric" //gel onu metric olarak değiştir, çünkü celsius olarak atamak istiyoruz.
        getWeather()
    }
})
...
document.body.addEventListener("load", async()=>{ //ekran yüklendiğinde gelsin async'le bir fonksiyon çalıştırsın ve await yapısıyla bekleyelim. 
    await getWeather()
})